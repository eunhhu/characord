...
test("state persists profiles, character presets, sessions, and pending input", async (t) => {
...
test("consuming a snapshot preserves messages queued later", async (t) => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "character-bot-state-"));
  t.after(() => rm(directory, { recursive: true, force: true }));
...
test("story reset preserves profiles and character preset", async (t) => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "character-bot-state-"));
  t.after(() => rm(directory, { recursive: true, force: true }));
  const state = new StateStore(path.join(directory, "state.json"));
  await state.load();
